...
To download the database files, find the most recent version on [tbb.bio.uu.nl/tina/CAT\_prepare/](https://tbb.bio.uu.nl/tina/CAT_prepare/), download and extract, and you are ready to go!
...
For a custom CAT database, you must have the following input ready before you launch a `CAT prepare` run.
...
$ CAT reads --mode mcr -b bin_folder/ -c contigs.fasta -1 forward_reads.fq.gz -2 reverse_reads.fq.gz -d db/ -t tax/ --c2c CAT_contig2classification_file.txt --b2c BAT_bin2classification_file.txt
```
...
$ CAT reads --mode mcr -b bin_folder/ -c contigs.fasta -1 forward_reads.fq.gz -2 reverse_reads.fq.gz -d db/ -t tax/ --c2c CAT_contig2classification_file.txt --b2c BAT_bin2classification_file.txt --alignment_unmapped unmapped_alignment_file.diamond
```
...
$ CAT add_names -i RAT.completete_abundance_file.txt -o RAT.completete_abundance_file_with_names.txt -t tax/ 
```
